...
class Down(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.down = nn.Sequential(nn.MaxPool2d(kernel_size=2, stride=2),
                                  Dilated_res_Conv(in_channels, out_channels))
...
class UPPNet(nn.Module):
    def __init__(self, n_ch, n_classes, ocr=False, check=True):
        ...
        self.input_layer = Dilated_res_Conv(n_ch, channels[0])
        ...
        self.Conv0_1 = Dilated_res_Conv(channels[0], channels[0])

        self.Up1_1 = Up_cat(channels[2], channels[1])
        self.Conv1_1 = Dilated_res_Conv(channels[1], channels[1])

        self.Up0_2 = Up_cat(channels[1], channels[0])
        self.Conv0_2 = Dilated_res_Conv(channels[0]*2, channels[0])

        self.Up2_1 = Up_cat(channels[3], channels[2])
        self.Conv2_1 = Dilated_res_Conv(channels[2], channels[2])

        self.Up1_2 = Up_cat(channels[2], channels[1])
        self.Conv1_2 = Dilated_res_Conv(channels[1]*2, channels[1])

        self.Up0_3 = Up_cat(channels[1], channels[0])
        self.Conv0_3 = Dilated_res_Conv(channels[0]*3, channels[0])

        self.Up3_1 = Up_cat(channels[4], channels[3])
        self.Conv3_1 = Dilated_res_Conv(channels[3], channels[3])

        self.Up2_2 = Up_cat(channels[3], channels[2])
        self.Conv2_2 = Dilated_res_Conv(channels[2]*2, channels[2])

        self.Up1_3 = Up_cat(channels[2], channels[1])
        self.Conv1_3 = Dilated_res_Conv(channels[1]*3, channels[1])

        self.Up0_4 = Up_cat(channels[1], channels[0])
        self.Conv0_4 = Dilated_res_Conv(channels[0]*4, channels[0])
